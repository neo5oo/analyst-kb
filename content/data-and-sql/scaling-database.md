...
quiz:
  ...
  passingScore: 3
  questions:
  - question: Что такое масштабирование базы данных?
    options:
    - Удаление старых данных из таблиц для экономии места
    - Увеличение производительности и емкости системы при росте нагрузки
    - Перевод базы данных из SQL в NoSQL
    - Создание резервной копии перед релизом
    correctIndex: 1
    explanation: Масштабирование БД нужно, чтобы система выдерживала рост данных, запросов и нагрузки.
  - question: Чем вертикальное масштабирование отличается от горизонтального?
    options:
    - Вертикальное добавляет новые серверы, горизонтальное увеличивает мощность одного сервера
    - Вертикальное увеличивает мощность одного сервера, горизонтальное добавляет новые серверы
    - Вертикальное используется только в NoSQL, горизонтальное только в SQL
    - Отличий нет, это два названия одного подхода
    correctIndex: 1
    explanation: При вертикальном масштабировании усиливают один сервер, а при горизонтальном добавляют новые узлы и распределяют нагрузку.
  - question: В каком случае репликация будет наиболее полезна?
    options:
    - Когда нужно масштабировать чтение и повысить отказоустойчивость
    - Когда нужно распределить запись между разными шардами
    - Когда нужно полностью отказаться от мастера
    - Когда нужно выполнить JOIN между разными серверами
    correctIndex: 0
    explanation: Репликация позволяет читать данные с реплик, но запись обычно все равно идет в мастер.
  - question: Когда шардирование становится оправданным решением?
    options:
    - Когда данных мало, а команда хочет сразу сделать сложную архитектуру
    - Когда нужно просто ускорить удаление старых записей внутри одной таблицы
    - Когда высокая нагрузка на запись или данные уже не помещаются на один сервер
    - Когда нужно сохранить все JOIN без изменений
    correctIndex: 2
    explanation: Шардирование распределяет данные между серверами и помогает масштабировать запись и объем хранения.
---
