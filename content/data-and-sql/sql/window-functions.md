...
weight: 60
draft: false
description: Оконные функции (Window Functions) — вычисляют значения (ранги, суммы, скользящие средние) по группе строк (окну), не сворачивая результат в одну строку (в отличие от GROUP BY).
quiz:
  title: Проверка знаний
  passingScore: 3
  questions:
  - question: Для чего в SQL нужны оконные функции?
    options:
    - Для создания таблиц
    - Для изменения прав доступа
    - Для аналитических вычислений по окну строк без схлопывания результата
    - Для маршрутизации сообщений
    correctIndex: 2
    explanation: Window functions — важный инструмент продвинутой аналитики.
  - question: Какие задачи часто решают оконные функции?
    options:
    - Создание foreign key
    - Ранги, накопительные суммы, lag/lead, сравнение с соседними строками
    - Определение типа столбца
    - Настройку TLS-сертификата
    correctIndex: 1
    explanation: Это типичный набор применений window functions.
  - question: Что обычно задаёт границы аналитического окна?
    options:
    - GRANT и REVOKE
    - LIMIT и OFFSET
    - BEGIN и COMMIT
    ...
    correctIndex: 3
    explanation: Эти операторы определяют разбиение и порядок в окне.
  - question: Почему оконные функции особенно ценны аналитику?
    options:
    - Они позволяют делать сложную аналитику в одном SQL-запросе без постобработки
    - Они заменяют все подзапросы всегда
    - Они работают только в NoSQL
    - Они запрещают JOIN
    correctIndex: 0
    explanation: Часто именно окна делают запрос и выразительным, и компактным.
---
...
Обычный SQL с `GROUP BY` может ответить на эти вопросы, но ценой потери деталей. Группировка свернет все строки в одну по группе, и вы не увидите каждого ученика. Вы можете использовать подзапросы, но они будут громоздкими и медленными.

**Оконные функции (window functions)** — это инструмент, который позволяет выполнять вычисления над набором строк, связанных с текущей строкой, но в отличие от `GROUP BY`, они не сворачивают строки в одну. Каждая строка остается в результате, и к ней добавляется результат оконного вычисления.
...
1. **Оконные функции выполняют вычисления над набором строк, связанных с текущей строкой, без сжатия результата.** Каждая строка остается, и к ней добавляется вычисленное значение. Это главное отличие от `GROUP BY`.

2. **Ключевое отличие от GROUP BY:** GROUP BY сворачивает группы в одну строку, оконные функции сохраняют все строки. Оконные функции — это мост между детальными данными и агрегированными.
...
6. **ROWS vs RANGE:** ROWS оперирует физическими позициями строк, RANGE — логическими значениями (все строки с одинаковым значением ORDER BY считаются одной группой). RANGE может давать неожиданные результаты при одинаковых значениях в ORDER BY.
